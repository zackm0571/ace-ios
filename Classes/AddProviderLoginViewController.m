//
//  AddProviderLoginViewController.m
//  linphone
//
...
@interface AddProviderLoginViewController ()
@property (weak, nonatomic) IBOutlet UITextField *textFieldUsername;
@property (weak, nonatomic) IBOutlet UITextField *textFieldPassword;
@property (weak, nonatomic) IBOutlet UITextField *textFieldPort;
@property (weak, nonatomic) IBOutlet UITextField *textFieldTransport;
@property (weak, nonatomic) IBOutlet UITextField *textFieldDomain;
@property (weak, nonatomic) IBOutlet UITextField *textFieldUserId;
@property (weak, nonatomic) IBOutlet UIButton *buttonAdvanced;
@property (weak, nonatomic) IBOutlet UIButton *buttonSelectProvider;
...
@end
@implementation AddProviderLoginViewController
{
...
- (id)init {
    self = [super initWithNibName:@"AddProviderLoginViewController" bundle:[NSBundle mainBundle]];
    if (self != nil) {
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [self.scrollView setContentSize:CGSizeMake(self.view.frame.size.width, self.view.frame.size.height + self.buttonAddAccount.frame.size.height * 2)];
}
...
- (void)viewDidLoad {
    ...
    [self.textFieldTransport.layer setBorderColor:[UIColor whiteColor].CGColor ];
    [self.textFieldTransport.layer setBorderWidth:1.0];
    [self.textFieldTransport.layer setCornerRadius:5];
    self.textFieldTransport.text = @"TCP";
    [self.textFieldTransport setDelegate:self];
...
- (void)initLoginSettingsFields {
    ...
    self.textFieldTransport.text = [[[DefaultSettingsManager sharedInstance].sipRegisterTransport uppercaseString] stringByReplacingOccurrencesOfString:@"\"" withString:@""];
...
NSString *LOGIN_INDEX_KEY_ = @"login_index";
- (void)setupProviderPickerView {
    ...
    NSInteger cachedSelection = [[NSUserDefaults standardUserDefaults] integerForKey:(NSString*)LOGIN_INDEX_KEY_];
...
- (void)registrationUpdate:(LinphoneRegistrationState)state message:(NSString *)message {
    switch (state) {
        ...
        case LinphoneRegistrationFailed: {
            [self.buttonAddAccount setEnabled:YES];
...
- (IBAction)onLoginClick:(id)sender {
    ...
    [[DefaultSettingsManager sharedInstance] setSipRegisterTransport:self.textFieldTransport.text];
...
UIAlertView *_transportAlert;
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    
    if([alertView isEqual:_transportAlert]){
        if(buttonIndex == 1){
            [self.textFieldTransport setText:@"TCP"];
            [self.textFieldPort setText:@"25060"];
        }
        else{
            [self.textFieldTransport setText:@"TLS"];
            [self.textFieldPort setText:@"25061"];
        }
        
        [self.textFieldTransport resignFirstResponder];
    }
    else{
...
- (void)didSelectUICustomPicker:(UICustomPicker *)customPicker selectedItem:(NSString*)item {
    ...
    if([[[[NSUserDefaults standardUserDefaults] dictionaryRepresentation] allKeys] containsObject:[NSString stringWithFormat:@"provider%ld_domain", (long)providerPickerView.selectedRow]]){
        ...
        [[NSUserDefaults standardUserDefaults] setInteger:providerPickerView.selectedRow forKey:(NSString*)LOGIN_INDEX_KEY_];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
...
- (void)apiSignIn {
    ...
    NSString *transport = [self.textFieldTransport.text uppercaseString];
...
- (IBAction)onToggleAdvanced:(id)sender {
    ...
    if(!isAdvancedShown){
        ...
        [self.advancedViewPanel setHidden:NO];
        isAdvancedShown = YES;
    }
    else{
        [toggle setTitle:@"Advanced" forState:UIControlStateNormal];
        [self.advancedViewPanel setHidden:YES];
        isAdvancedShown = NO;
    }
}
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    _transportAlert = [[UIAlertView alloc] initWithTitle:@"Select Transport" message:@"" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"TCP", @"TLS", nil];
    [_transportAlert show];
...
- (IBAction)onAddAccount:(id)sender {
    [self.buttonAddAccount setEnabled:NO];
    NSString *rueConfigFormatURL = @"_rueconfig._tls.%domain%";
